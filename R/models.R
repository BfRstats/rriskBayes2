...
#' specificity takes a form for misclassifications at the individual level 
...
#'       x ~ dbin(ap,n)
#'    }}
#' for misclassifications at the individual level with fixed sensitivity 
#' resp. specifity (\code{k=1} and \code{misclass="individual-fix-sp"}. 
#' Also, both can be set to fixed values)
#' \preformatted{model{
#'        pi ~ dbeta(prior.pi[1], prior.pi[2])
#'
...
#' kth iteration will be used for inference, where k is the value of thin. 
...
#' distribution used as prior for the Poisson parameter \code{lambda}, e.g. \cr 
#' \code{lambda} ~ \code{prior.lambda(*,*)=unif(*,*)}
#' @param prior.pi numeric vector containing parameters of a beta distribution
#' describing prior knowledge about prevalence (proportion of contaminated 
#' samples), e.g. \cr \code{pi} ~ \code{prior.pi(*,*)=beta(*,*)}
#' @param simulation logical, value \code{TRUE} means the function will be 
#' called within any simulation routine, in this case the graphical diagnostic 
#' interface will not be invoked (default \code{FALSE})
...
#' @param simulation logical, value \code{TRUE} means the function will be called within any simulation 
#' routine, in this case the graphical diagnostic interface will not be invoked (default \code{FALSE})
